...
$("#queryCategoryLevel1").change(function(){
	var level1 = $("#queryCategoryLevel1").val();
	if(level1 != '' && level1 != null){
		$.ajax({
			type:"GET",//请求类型
			url:"categorylevelList.json",//请求的url
			data:{pid:level1},//请求参数
	...
	}else{  //当一级分类选择 “请选择时” =null
		$("#queryCategoryLevel2").html();  //先将二级分类清空
		var options = "<option value=''>--请选择--</option>";   //赋空值
		$("#queryCategoryLevel2").html(options);
	}
	//最后不管一级分类怎样选择，三级分类都要清空，因为三级分类只与二级分类有关
...
$("#queryCategoryLevel2").change(function(){
	var level2 = $("#queryCategoryLevel2").val();
	if(level2 != '' && level2 != null){
		$.ajax({
			type:"GET",//请求类型
			url:"categorylevelList.json",//请求的url
			data:{pid:level2},//请求参数
			dataType:"json",//ajax接口（请求url）返回的数据类型
			success:function(data){//data：返回数据（json对象）
				$("#queryCategoryLevel3").html("");
				var options = "<option value=''>--请选择--</option>";
